    ...
    try:
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                cursor.execute(
                    "CREATE TABLE items (itemid SERIAL PRIMARY KEY, type text, subtype text, size "
                ...
                stmt_str = (
                    'INSERT INTO items (type, subtype, size, gender, price, color, condition, brand, "desc", posted, photolink, status)'
                ...
                cursor.execute(
                    "CREATE TABLE sellers " + '("netid" text, "itemid" text)'
                )
                cursor.execute(
                    "INSERT INTO sellers " + "(netid, itemid) VALUES " + "('ishirai',0)"
                )
