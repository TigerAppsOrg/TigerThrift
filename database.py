    ...
    try:
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                stmt_str = (
                    "SELECT exists (SELECT 1 FROM users WHERE netid = %s LIMIT 1);"
                )
                ...
                if not is_user:
                    ...
                    stmt_str = "INSERT INTO users (netid, email, joined, phone, first_name, last_name, full_name, reservation_count) VALUES (%s, %s, %s, %s, %s, %s, %s, 0)"
                    cursor.execute(
                        stmt_str,
                        [
                            user_info["netid"],
    ...
    try:
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                user_info["phone"] = get_user_phone(
                    netid
                )  # will either be unknown or the phone number itself
                connection.commit()
                return user_info
    ...
    try:
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                stmt_str = (
                    "SELECT status, prodname, sellernetid from items where itemid = %s"
                )
                ...
                return (
                    ...
                    seller_email,
                    seller_phone,
                    titlecase(str(prodname)),
                )

    except Exception as ex:
        print(ex, file=stderr)
        if (
            str(ex) == "item already reserved"
            or str(ex) == "cannot find item info"
            or str(ex) == "item unavailable for reservation"
            or str(ex) == "cannot find sellerid"
            or str(ex) == "max number of reservations reached"
        ):
            return str(ex)
        return False
    ...
    try:
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                stmt_str = (
                    "INSERT INTO sellers " + "(netid, itemid) " + "VALUES (%s, %s)"
                )
    ...
    try:
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                item_name = info[1]
                stmt_str = (
                    "SELECT first_name, full_name, email FROM users WHERE netid=%s;"
                )
                cursor.execute(stmt_str, [sellernetid])
                row = cursor.fetchone()
                seller_first_name = row[0]
    ...
    try:
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                stmt_str = (
                    "UPDATE reservations SET completedtime = %s WHERE itemid = %s"
                )
...
def days_between(d1, d2):
    ...
    if "day" not in str(time_left):
        ...
        print(
            "YOUR RESERVATION IS ABOUT TO EXPIRE! Only", str(hours_left), "hours left!"
        )
        ...
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                for item_id in item_ids:
                    ...
                    now = datetime.utcnow()
                    dt = now.strftime(f)
                    time_left_to_complete_reservation = days_between(
                        dt, item_ids[item_id]
                    )
                    reservation_time_left = "".join(time_left_to_complete_reservation)
                    # if reservation_time_left == "YOUR RESERVATION HAS EXPIRED! 0 days left":
            ...
            with closing(connection.cursor()) as cursor:
                ...
                for item_id in item_ids:
                    ...
                    purchased_date = datetime.strptime(
                        str(item_ids[item_id]), "%Y-%m-%d %H:%M:%S"
                    )
        ...
        with connect(DATABASE_URL, sslmode="require") as connection:
            with closing(connection.cursor()) as cursor:
                ...
                if filter:
                    ...
                    if (
                        "subtype" in filter
                        and filter["subtype"] is not None
                        and filter["subtype"] != ""
                    ):
                    ...
                    if (
                        "gender" in filter
                        and filter["gender"] is not None
                        and filter["gender"] != ""
                    ):
                        stmt_str += "AND (gender = %s or gender = 'Unisex') "
                        cmd_args.append(filter["gender"])
                    if (
                        "brand" in filter
                        and filter["brand"] is not None
                        and filter["brand"] != ""
                    ):
                    ...
                    if (
                        "condition" in filter
                        and filter["condition"] is not None
